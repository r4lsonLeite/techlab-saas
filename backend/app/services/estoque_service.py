...
class EstoqueService:
    ...
    def reservar_peca(db: Session, produto_id: int, quantidade: int, loja_id: int, usuario_id: int, os_id: int = None):
        ...
        # 👇 A NOVA REGRA: Se for serviço, apenas retorna ele (sem mexer em estoque)
        if produto.is_servico:
            return produto

        # Se for peça física, faz a matemática e bloqueia se faltar
    ...
    def devolver_reserva(db: Session, produto_id: int, quantidade: int, usuario_id: int, os_id: int = None):
        ...
        if produto and not produto.is_servico: # 👈 Não devolve reserva de serviço, porque nunca foi reservado
    ...
    def efetivar_baixa(db: Session, produto_id: int, quantidade: int, usuario_id: int, venda_id: int, is_os: bool = False):
        ...
        # 👇 Se for serviço, deixa passar direto para a nota/venda
        if produto.is_servico:
            return produto
