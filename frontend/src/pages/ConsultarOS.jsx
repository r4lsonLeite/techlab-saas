...
export default function ConsultarOS({ cargo, osIdParaAbrir, setOsIdParaAbrir, abrirPDVComOS }) {
  ...
  // 🟢 ESTADOS DE NEGOCIAÇÃO (CARRINHO)
  const [obsBalcao, setObsBalcao] = useState("");
  const [valorDigitado, setValorDigitado] = useState("");
  const [pecasNegociacao, setPecasNegociacao] = useState([]); // Carrinho local
  const [produtosCatalogo, setProdutosCatalogo] = useState([]); // Todos os produtos da loja
  const [termoBuscaProduto, setTermoBuscaProduto] = useState(""); // Busca no modalzinho
  
  const isTecnico = String(cargo).toLowerCase().trim() === 'tecnico';

  useEffect(() => {
    carregarOrdens();
    // 🟢 Carrega os produtos da loja uma vez para podermos pesquisar na hora da negociação
    carregarProdutos();
  }, [osIdParaAbrir]); 

  // Quando uma OS for selecionada, copiamos as peças dela para o nosso "carrinho" local
  useEffect(() => {
  ...
  // Se o carrinho mudar (adicionar/remover peça), recalcula o valor sugerido
  useEffect(() => {
  ...
  // 🟢 FUNÇÕES DO CARRINHO DE NEGOCIAÇÃO
  ...
  const handleAtualizarStatus = async (novoStatus) => {
    ...
    if (novoStatus.includes('APROVADO')) {
      ...
      // 🟢 Envia a nova lista de peças para o Backend salvar/reservar/devolver
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-[#1e293b] p-6 rounded-2xl border border-slate-700 shadow-lg">
                ...
                <div className="flex items-center gap-3"><span className="text-white font-medium text-lg">{osAtiva.cliente_nome}</span><button onClick={() => verPerfilCliente(osAtiva.cliente_id)} className="px-2 py-1 bg-blue-500/20 text-blue-400 rounded-lg font-bold text-xs">🔍 Ver Perfil</button></div>
                <p className="text-slate-400 mt-2">{osAtiva.telefone}</p>
              </div>
              <div className="bg-[#1e293b] p-6 rounded-2xl border border-slate-700 shadow-lg">
                ...
                <p className="text-slate-400 mt-2">Valor: R$ {Number(osAtiva.valor_orcamento || 0).toFixed(2)}</p>
              </div>
            </div>
            ...
            <div className="bg-[#1e293b] p-6 rounded-2xl border border-purple-500/30 shadow-lg">
              ...
              <div className="bg-[#0f172a] rounded-xl border border-slate-700 overflow-hidden mb-4">
                <table className="w-full text-left text-sm text-slate-300">
                  <thead className="bg-slate-800 text-slate-400 text-xs uppercase">
                    <tr><th className="p-3">Qtd</th><th className="p-3">Descrição</th><th className="p-3 text-right">Valor Unit.</th><th className="p-3 text-right">Total</th>{!isTecnico && <th className="p-3 text-center">Ações</th>}</tr>
                  </thead>
                  <tbody className="divide-y divide-slate-700">
                    {pecasNegociacao.length === 0 ? (<tr><td colSpan="5" className="p-4 text-center text-slate-500">Nenhum item</td></tr>) : (
                      pecasNegociacao.map((item, idx) => {
                        ...
                        return (
                          <tr key={idx} className="hover:bg-slate-800/50">
                            ...
                            <td className="p-3 text-right font-bold text-emerald-400">R$ {(qtd * preco).toFixed(2)}</td>
                            {!isTecnico && (
              ...
              {/* 🟢 BARRA DE PESQUISA PARA A ATENDENTE ADICIONAR ITENS */}
            ...
            {/* APROVAÇÃO (MOSTRA APENAS SE ESTIVER AGUARDANDO) */}
            {(osAtiva.status === 'Aguardando Cliente' || osAtiva.status === 'Aguardando Reavaliação') && !isTecnico && (
              <div className="bg-[#1e293b] p-8 rounded-2xl border border-purple-500/40 shadow-xl mt-8">
                <h3 className="text-white font-bold text-xl mb-6 border-b border-slate-700 pb-3">📞 Finalizar Orçamento</h3>
                <div className="mb-6">
                  <label className="block text-slate-400 text-sm font-bold mb-2">Observações para o Técnico</label>
                  <textarea value={obsBalcao} onChange={(e) => setObsBalcao(e.target.value)} className="w-full p-4 rounded-xl bg-[#0f172a] text-white border-2 border-slate-600 focus:border-purple-500 resize-none" rows="2" />
                </div>
                <div className="flex gap-6 items-end">
                  <div className="flex-1">
                    <label className="block text-slate-400 text-sm font-bold mb-3">Valor Final Negociado (R$)</label>
                    <input type="number" value={valorDigitado} onChange={(e) => setValorDigitado(e.target.value)} className="w-full p-4 rounded-xl bg-[#0f172a] text-white text-xl border-2 border-emerald-500" />
                  </div>
                  <button onClick={() => handleAtualizarStatus('APROVADO - Fila de Conserto')} className="bg-emerald-600 text-white px-8 py-4 rounded-xl font-bold text-lg">✅ Aprovar</button>
                  <button onClick={() => handleAtualizarStatus('Recusado - Devolver ao Cliente')} className="bg-red-600 text-white px-8 py-4 rounded-xl font-bold text-lg">❌ Recusar</button>
                </div>
              </div>
            )}
            
            {/* BOTÕES DE PDV PARA ENTREGA */}
            {osAtiva.status === 'Pronto para Retirada' && !isTecnico && (
              <div className="bg-emerald-500/10 p-6 rounded-2xl border border-emerald-500/30 flex justify-between items-center mt-6">
                <div><h3 className="text-emerald-400 font-bold">✅ Aparelho Pronto</h3><p className="text-slate-300">Aguardando Pagamento e Retirada</p></div>
                <button onClick={() => abrirPDVComOS(osAtiva)} className="bg-blue-600 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2">📦 Ir para o Pagamento</button>
              </div>
            )}
